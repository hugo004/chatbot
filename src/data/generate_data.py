...
def extract_word_info(sentence: str, word: Union[List[str], str], label: str):
...
def generate_ner_data():
    training_data = []
    data = json.loads(
        open(os.path.join(PROJECT_ROOT_PATH, 'data/ner-train-raw.json')).read())
...
def get_intents():
    general_intent = json.loads(open(os.path.join(
        PROJECT_ROOT_PATH, 'data/intents/general-intents.json')).read())
    booking_intent = json.loads(open(os.path.join(
        PROJECT_ROOT_PATH, 'data/intents/booking-intents.json')).read())
    eform_intent = json.loads(
        open(os.path.join(PROJECT_ROOT_PATH, 'data/intents/eform-intents.json')).read())
...
def generate_chatbot_data(show_detail: bool, name="chatbot-intents", intents: list = get_intents()):
    ...
    pickle.dump(words, open(os.path.join(
        PROJECT_ROOT_PATH, f'models/{name}-words.pkl'), 'wb'))
    pickle.dump(labels, open(os.path.join(
        PROJECT_ROOT_PATH, f'models/{name}-labels.pkl'), 'wb'))
